...
class Wrapper():
                ...
                for i in range(n_a):
                    ...
                    if found:
                        valid_pair = np.append(
                            valid_pair, [[cand_a[i][3], cand_b[max_j][3], max_score]], axis=0)
            ...
            if k not in invalid_pairs:
                ...
                for i in range(len(valid_pairs[k])):
                    ...
                    if found == 1:
                        ...
                        personwise_keypoints[person_idx][-1] += self.keypoints_list[part_Bs[i].astype(
                    ...
                    # if find no partA in the person keypoints, add one person
                        ...
                        row[-1] = sum(self.keypoints_list[valid_pairs[k]
                                                          [i, :2].astype(int), 2]) + valid_pairs[k][i][2]
                        personwise_keypoints = np.vstack(
                            [personwise_keypoints, row])
    ...
    def multiple_detections(self, frame):
        ...
        for part in range(self.n_points):
            ...
            for i in range(len(keypoints)):
                ...
                self.keypoints_list = np.vstack(
                    [self.keypoints_list, keypoints[i]])
                keypoint_id += 1
        ...
        personwise_keypoints = self.get_personwise_keypoints(
        ...
        for i in range(self.n_points-1):
            for n in range(len(personwise_keypoints)):
                ...
                cv.circle(frame, (B[0], A[0]), 4,
                          self.colors[i], -1, cv.FILLED)
                cv.circle(frame, (B[1], A[1]), 4,
                          self.colors[i], -1, cv.FILLED)
                cv.line(frame, (B[0], A[0]), (B[1], A[1]),
                        self.colors[i], 3, cv.LINE_AA)
    ...
    def single_detection(self, frame):
        ...
        for i in range(15):  # I want only right shoulder(2), neck(1) and head(0) keypoints
            ...
            if prob > self.threshold:
                ...
                cv.circle(frame, (x, y), 3,
                          self.colors[i], thickness=-1, lineType=cv.FILLED)
            else:
        ...
        for x in range(self.n_points-1):
            ...
            cv.line(frame, points[par1], points[par2],
                    self.colors[x], 3, cv.LINE_AA)
    ...
    def run_simulation(self):
        ...
        self.in_height = 368
        self.in_width = int(
            (self.in_height/self.frame_height)*self.frame_width)
